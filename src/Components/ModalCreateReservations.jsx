...
const ModalCreateReservations = ({ open, setOpen,fetchReservations }) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await ReservationsService.createReservation(reservations);
            fetchReservations();
    ...
    return <Modal show={open} onHide={handleClose} position="center">
        <Modal.Header closeButton>
            <Modal.Title className="text-dark">Crée une réservation</Modal.Title>
        </Modal.Header>
        <Form className="text-dark" onSubmit={handleSubmit}>
            <Modal.Body>
                <Form.Group className="mb-3" controlId="formBasicEmail">
                    <Form.Label>Date d'arrivé</Form.Label>
                    <Form.Control type="datetime-local" placeholder="ex: 24/03/2025 14:00" value={reservations.check_in_date} name="check_in_date" onChange={handleChange} />
                </Form.Group>
                <Form.Group className="mb-3" controlId="formBasicEmail">
                    <Form.Label>Date de départ</Form.Label>
                    <Form.Control type="datetime-local" placeholder="ex: 24/03/2025 14:00" value={reservations.check_out_date} name="check_out_date" onChange={handleChange} />
                </Form.Group>
                <Form.Group className="mb-3" controlId="formBasicEmail">
...
export default ModalCreateReservations;